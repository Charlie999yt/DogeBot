...
module.exports = Smruti = async (Smruti, mek) => {
  try {
...
const body = (type === 'listResponseMessage' && mek.message.listResponseMessage.title) ? mek.message.listResponseMessage.title : (type === 'buttonsResponseMessage' && mek.message.buttonsResponseMessage.selectedButtonId) ? mek.message.buttonsResponseMessage.selectedButtonId : (type === 'conversation' && mek.message.conversation.startsWith(prefix)) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption.startsWith(prefix) ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption.startsWith(prefix) ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text.startsWith(prefix) ? mek.message.extendedTextMessage.text : (type == 'stickerMessage') && (getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== null && getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== undefined) ? getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) : ""
    ...
    const botNumber = Smruti.user.jid;
    
    const botNumberss = Smruti.user.jid + "@c.us";
...
// 𝘼𝙪𝙩𝙤 𝙍𝙚𝙖𝙙 𝙂𝙘
var ampun = await Smruti.chats.array.filter(v => v.jid.endsWith('g.us'))
ampun.map( async ({ jid }) => {
if (readGc === false) return
await Smruti.chatRead(jid)
})
// 𝘼𝙪𝙩𝙤 𝙍𝙚𝙖𝙙 𝙋𝙘
var chatss = await Smruti.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
...
await Smruti.updatePresence(from, Presence.recording)
} else if (autoketik) {
	if (autoketik === false) return
await Smruti.updatePresence(from, Presence.composing)
}
...
const checkRegisteredUser = (sender) => {
    ...
    let senderr = mek.key.fromMe
      ...
      ? mek.participant
      : mek.key.remoteJid;
    const totalchat = await Smruti.chats.all();
    const ownerNumber = [`919556416715@s.whatsapp.net`]
	const senderNumber = sender.split("@")[0] 
    const m = simple.smsg(Smruti, mek);
    const groupMetadata = isGroup ? await Smruti.groupMetadata(from) : "";
   ...
   const isRegister = register.includes(sender)
    ...
    const conts = mek.key.fromMe
      ? Smruti.user.jid
      : Smruti.contacts[sender] || { notify: jid.replace(/@.+/, "") };
...
cmhit.push(command)
  ...
  /*const reply = (teks) => {
      Smruti.sendMessage(from, teks, text, { quoted: mek });
    };*/
    const textImg = (teks) => {
           return ikyy.sendMessage(from, teks, text, {quoted: freply, thumbnail: fs.readFileSync('./Smruti.jpg')})
        }
const math = (teks) => {
           return Math.floor(teks)
       }
/*const reply = (teks) => {
			Smruti.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/919556416715?text=hello`}}})
		}*/
    const sendMess = (hehe, teks) => {
      Smruti.sendMessage(hehe, teks, text);
    };

    const mentions = (teks, memberr, id) => {
      id == null || id == undefined || id == false
        ? Smruti.sendMessage(from, teks.trim(), extendedText, {
...
const freply = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { "contactMessage": { "displayName": `${pushname}`, "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:XL;${pushname},;;;\nFN:${pushname},\nitem1.TEL;waid=${senderr.split('@')[0]}:${senderr.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, "jpegThumbnail":fs.readFileSync('./Smruti.jpg')
        }}}
...
const ftex = {
    ...
    const fakestatus = (teks) => {
      Smruti.sendMessage(from, teks, text, {
        quoted: {
          key: {
            fromMe: false,
            participant: `0@s.whatsapp.net`,
    ...
    const fakethumb = (teks, yes) => {
      Smruti.sendMessage(from, teks, image, {
...
Smruti.on('CB:action,,battery', json => {
...
download(url, "./stik" + names + ".png", async function () {
        ...
        exec(
          ...
          (err) => {
            ...
            Smruti.sendMessage(to, media, MessageType.sticker, { quoted: mek });
...
if (budy.toLowerCase() === `8473`){
		if (isRegister) return 
		    ...
		    Smruti.sendMessage(from, teks, text, {quoted: fkontak })
...
key: {
    ...
    const sendMediaURL = async (to, url, text = "", mids = []) => {
      ...
      download(url, filename, async function () {
        ...
        Smruti.sendMessage(to, media, type, {
          quoted: mek,
          mimetype: mime,
          caption: text,
...
Smruti.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
///Button Image
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await Smruti.prepareMessage(from, kma, image)
...
Smruti.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Video
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await Smruti.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await Smruti.prepareMessage(from, kma, location)
const buttonMessages = {
...
Smruti.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const kick = function (from, orangnya) {
      for (let i of orangnya) {
        Smruti.groupRemove(from, [i]);
      }
    };
    const add = function (from, orangnya) {
      Smruti.groupAdd(from, orangnya);
    };
        const grupinv = (teks) => {
        	grup = Smruti.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Smruti Bot Inc.`, "footerText": "*_WHATSAPP BOT_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
            Smruti.relayWAMessage(grup)
        }
const fakeitem = (teks) => {
            Smruti.sendMessage(from, teks, text, {
                quoted: {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`WHATSAPP BOT`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
             res = Smruti.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*Smruti*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
             Smruti.relayWAMessage(res)
        }
const fakewa = (teks) => {        
Smruti.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Smruti`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/919556416715?text=hello`}}})
		}
const replyfakelink = (teks) => {
Smruti.sendMessage(from, teks, text,{contextInfo :{text: 'hi',
"forwardingScore": 1000000000,
isForwarded: false,
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\nWHATSAPP BOT. (@nnda.krnwn_) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/919556416715`,
                "mediaType": "10",
                "mediaUrl": `wa.me/919556416715`,
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/919556416715`,
...
// Support Di Versi 3.5.2 
    const sendBug = async (target, teks) => {
      if (!teks) teks = ".";
      await Smruti.relayWAMessage(
        Smruti.prepareMessageFromContent(
          target,
          Smruti.prepareDisappearingMessageSettingContent(0),
          {}
        ),
        { waitForAck: true }
      );
      Smruti.sendMessage(target, teks, "conversation");
    };
...
async function sendFileFromUrl(from, url, caption, msg, men) {
            ...
            return Smruti.sendMessage(from, await getBuffer(url), type, {caption: caption, quoted: mek, mimetype: mime, contextInfo: {"mentionedJid": men ? men : []}})
        }
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : "919556416715@s.whatsapp.net";
    ...
    if (isGroup && isAntilink && !mek.key.fromMe) {
      if (budy.includes("://chat.whatsapp.com/")) {
        ...
        Smruti.groupRemove(from, [sender]);
      }
    }
    if (isGroup && isAntiviewonce && m.mtype == "viewOnceMessage") {
      ...
      Smruti.copyNForward(m.chat, msg);
    }
    ...
    //auto voice note by Smruti
    for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					Smruti.sendMessage(from, result, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek})
					}
			}
			//auto sticker
			for (let anji of setik){
				if (budy === anji){
					...
					Smruti.sendMessage(from, result, sticker, { quoted: mek})
					}
			}
			//auto image
			for (let anjh of imagi){
				if (budy === anjh){
					...
					Smruti.sendMessage(from, result, image, { quoted: mek})
					}
			}
			// auto text 
...
await Smruti.setStatus(`Bot Mode On Since ${uptime}`).catch((_) => _);
...
const serialUser = createSerial(18)
	         try {
								ppimg = await Smruti.getProfilePicture(`${sender.split('@')[0]}@c.us`)
								} catch {
...
*「 WHATSAPP BOT 」*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `menu`,buttonText:{displayText: `</️MENU`},type:1}]
              imageMsg = (await Smruti.prepareMessageMedia(haitod, "imageMessage", { thumbnail: haitod, })).imageMessage
              buttonsMessage = {footerText:'Smruti', imageMessage: imageMsg,
              contentText:`${anuu}`,buttons,headerType:4}
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{quoted: fkontak})
              Smruti.relayWAMessage(prep)
	         console.log(color('[REGISTER]'), color(time, 'yellow'), 'Serial:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
	    // console.log(e)
            setTimeout( () => {
			Smruti.updatePresence(from, Presence.composing)
			reply(`*Thank you for being registered WHATSAPP BOT*`)
		}, 2000)
        break
      
case 'help':
case 'menu':
groups = Smruti.chats.array.filter(v => v.jid.endsWith('g.us'))
              privat = Smruti.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
              ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
           
              uptime = process.uptime();
              timestampe = speed();
              totalChat = await Smruti.chats.all()
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       txtt =`Hi ${pushname} 👋\n\nIF YOU ARE USING MOD WHATSAPP & BUTTONS DOESNT SHOW, TYPE #allmenu`
               ...
               imageMsg = (await Smruti.prepareMessageMedia(fs.readFileSync(`./stik/oksip.jpeg`), 'imageMessage', {thumbnail: fs.readFileSync(`./stik/oksip.jpeg`)})).imageMessage
...
groups = Smruti.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "919556416715@s.whatsapp.net"
        dtod = '919556416715'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
       stst = await Smruti.getStatus(`${sender.split('@')[0]}@c.us`)
...
case 'credit':
 ...
 🌿 : wa.me/${dtod2.split('@')[0]}
 `
     credit = Smruti.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 1000, "message": `${yez}`, "footerText": "hehe", "thumbnail": iye, "surface": 'CATALOG'}}, {quoted: mek})
            Smruti.relayWAMessage(credit)
        break
case 'suit':
...
case 'bugreport':
              ...
              Smruti.sendMessage('919556416715@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                ...
                Smruti.sendMessage(from, dapuhy, image, {quoted: mek})        
                break    
case 'auu': // by Smruti
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       reply('No Sis')
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
               ...
               prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{})
               Smruti.relayWAMessage(prep)
               break     
...
Smruti.sendMessage(from, { "degreesLatitude": '', "degreesLongitude": '', "jpegThumbnail": iye}, MessageType.location, {quoted: ikymemek})
           break
case 'mengyoi':
if (!mek.key.fromMe) return;
sendBug(from)
...
case 'buglink':
                if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
                 Smruti.toggleDisappearingMessages(from, 0)
               Smruti.sendMessage(from, virtex3(prefix), text, { quoted:ftrol, contextInfo :{text: '🔥',
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
                ...
                "previewType": "PHOTO",
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": fs.readFileSync(`./Smruti.jpg`),
...
Smruti.sendMessage(from, listMsg, MessageType.listMessage, {quoted:ftrol})
Smruti.toggleDisappearingMessages(from, 0)
break
case 'bugcatalog':
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
Smruti.toggleDisappearingMessages(from, 0)
 babi = (teks) => {
             res = Smruti.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 9999999, "message": teks, "footerText": `${emoji2(prefix)}`, "thumbnail": iye, "surface": 'CATALOG' }}, {quoted: {
  key: {
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
                                    if (error) return reply(mess.error.api)
									 Smruti.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: freply})
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
		...
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			Smruti.sendMessage(from, damdu, sticker, {quoted: mek})
			break
 case 'owner3':
let ini_list = []
for (let i of nomorowner1) {
const vname = Smruti.contacts[i] != undefined ? Smruti.contacts[i].vname || Smruti.contacts[i].notify : undefined
ini_list.push({
"displayName": 'Smruti',
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname ? `${vname}` : `${Smruti.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
hehe = await Smruti.sendMessage(from, {
"displayName": `${ini_list.length} contact`,
"contacts": ini_list 
}, 'contactsArrayMessage', { quoted: mek })
Smruti.sendMessage(from,'This is my owners contact',text,{quoted: mek})
break
case 'command':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
 listMsg = {
...
Smruti.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]},quoted:ftrol})
break
case 'downloadmenu':
...
㋛${prefix}𝙩𝙬𝙞𝙩𝙩𝙚𝙧 <𝙪𝙧𝙡>
`
sendButMessage(from, menu, `WHATSAPP BOT`, [
          {
            buttonId: `command`,
            buttonText: {
...
break
case 'catalog':
      list = await Smruti.prepareMessageFromContent(from, {
    "listMessage": {
      "title": "PAYMENT",
      "description": "CLICK",
      "buttonText": "",
  ...
  Smruti.relayWAMessage(list, {waitForAck: true})
       
case 'triggered':
                
                if (isImage || isQuotedImage) {
                    let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                    let media = await Smruti.downloadAndSaveMediaMessage(encmedia)
                ...
                exec(`webpmux -set exif ./sticker/data.exif ./sticker/triggered.webp -o ./sticker/triggered.webp`, async (error) => {
                    if (error) return reply(mess.error.api)
                    Smruti.sendMessage(from, fs.readFileSync(`./sticker/triggered.webp`), sticker, {quoted: freply})
...
case 'script':
		case 'sc':
		case 'sourcecode':
		Smruti.sendMessage(from, { text: "https://github.com", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
...
if (args.length < 1) return reply('The text?')
                    teks = body.slice(10)
                    Smruti.sendMessage('status@broadcast', teks, MessageType.text)
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
  if (args.length < 1) return reply('The text?')
                    teks = body.slice(12)
                    Smruti.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`${fakeyoi}`}, MessageType.location)
...
if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						...
						buff = await Smruti.downloadMediaMessage(encmedia)
						Smruti.sendMessage('status@broadcast', buff, sticker)
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await Smruti.downloadMediaMessage(encmedia)
						Smruti.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await Smruti.downloadMediaMessage(encmedia)
						Smruti.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					var mediap = await Smruti.downloadAndSaveMediaMessage(enmediap)
                    const buffer3 = fs.readFileSync(mediap)
                    Smruti.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 359996400, caption: `${konti}`})
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					media = await Smruti.downloadAndSaveMediaMessage(enmedia)
                    const buffer6 = fs.readFileSync(media)
                    Smruti.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
                    ...
                    Smruti.sendMessage('status@broadcast', buffer, MessageType.image, {quoted: mek, caption: `${teksyy}`})
                    reply(`Image upload success:\n${teksyy}`)
                        break
					case 'shutdown':
					if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
				return Smruti.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
      case "delcmd":
        if (!isQuotedSticker)
...
for(let i of bancht){
met = await Smruti.groupMetadata(i)
teks += 'Id : ' + i + '\n'
...
nm = body.slice(7)
					...
					delb = await Smruti.downloadMediaMessage(boij)
					vien.push(`${nm}`)
					fs.writeFileSync(`./media/vn/${nm}.mp3`, delb)
					fs.writeFileSync('./database/vien.json', JSON.stringify(vien))
					Smruti.sendMessage(from, `Success, please check with *${prefix}vnlist*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Smruti.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
					// add sticker
					case 'addsticker':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					Smruti.sendMessage(from, `Success, please check with *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
					case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Smruti.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					// add image
					case 'addimage':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					Smruti.sendMessage(from, `Success, please check with *${prefix}imagelist*`, MessageType.text, { quoted: mek })
					break
					case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Smruti.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
					// add respond
					case 'addrespond':
			if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
...
case 'linkgroup':
              ...
              if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
				if (!isGroup) return reply('this feature is only for groups')
                   if (!isBotGroupAdmins) return reply('bot not admin')
					linkgc = await Smruti.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\n${groupName} *Group Link*`
					Smruti.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
                  case 'resetgrouplink':
         ...
         Smruti.query({json, expect200: true})
...
await Smruti.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
					case 'opengc':
					if (!isGroup) return reply('this feature is only for group')
                   ...
                   reply(`Successful opening group ${groupName}`)
						Smruti.groupSettingChange(from, GroupSettingChange.messageSend, false)
						break
						case 'closegc':
						if (!isGroup) return reply('this feature is only for group')
						if (!isGroupAdmins) return reply('this feature is only for admins')
                   if (!isBotGroupAdmins) return reply('bot is not admin')
						...
						Smruti.groupSettingChange(from, GroupSettingChange.messageSend, true)
					break
					//extra features
					case "pubglogo":
     ...
     Smruti.sendMessage(from, buffer, image, {quoted: mek, caption: 'Here'})
     break
             case "bc":  
        case "bcimage":  
        case "broadcast":  
                   if (!isOwner && !mek.key.fromMe) return reply('only owner can use this feature')    
					...
					ini_bc = args.join(' ')
					anu = await Smruti.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
					const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
					buff = await Smruti.downloadMediaMessage(encmedia)
					for (let _ of anu) {
						Smruti.sendMessage(_.jid, buff, image, {quoted:fakestatus ,caption: `「  *WHATSAPP BOT BROADCAST* 」\n\n${ini_bc}`})
					}
					reply('```Success Broadcast```')
					} else {
					for (let _ of anu) {
					Smruti.sendMessage(_.jid, `*WHATSAPP BOT BROADCAST*\n${ini_bc}`, text, { contextInfo: { mentionedJid: [sender],"forwardingScore":999,"isForwarded":true},sendEphemeral: true })
					}
					reply('```Success Broadcast```')
					}
					break
...
*[Wait] Wait a moment..*`
				...
				Smruti.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted:freply, caption: 'Here bro'})
					break  
        //tiktok
        case 'tiktoknowm':   case 'tiktok':
									...
									.then(result => {
										...
										.then(async (a) => {
											...
											me = `*Link* : ${a.data}`
											Smruti.sendMessage(from,meno,MessageType.video,{mimetype:'video/mp4',quoted: mek, caption:me})
											})
									...
									.then(result => {
										...
										.then(async (a) => {
											...
											Smruti.sendMessage(from,audin,MessageType.audio,{mimetype:'audio/mp4',quoted: mek})
											})
										}).catch((err) => reply(`Invalid link`))
									break
					// ml hero
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  gayy = body.slice(1)
					...
					Smruti.sendMessage(from, 'Question : *'+gayy+'*\n\nAnswer : '+ yag+'%', text, { quoted: mek })
					break
                case 'lesbicheck':
                case 'lesbiancheck':
					// source code by Smruti⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  lesbii = body.slice(1)
					...
					Smruti.sendMessage(from, 'Question : *'+lesbii+'*\n\nAnswer : '+ bi+'%', text, { quoted: mek })
					break
                case 'handsomecheck':
					// source code by Smruti⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
ganteng = body.slice(1)
					...
					Smruti.sendMessage(from, 'Question : *'+ganteng+'*\n\nAnswer : '+ teng+'%', text, { quoted: mek })
					break
		case 'beautycheck':
					// source code by Smruti⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
cantik = body.slice(1)
					...
					Smruti.sendMessage(from, 'Question : *'+cantik+'*\n\nAnswer : '+ tik+'%', text, { quoted: mek })
					break
		case 'character':
					// source code by Smruti⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
watak = body.slice(1)
					...
					Smruti.sendMessage(from, 'Question : *'+watak+'*\n\nAnswer : '+ tak, text, { quoted: mek })
				        break
                case 'can':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Smruti.sendMessage(from, 'Question : *'+bisakah+'*\n\nAnswer : '+ keh, text, { quoted: mek })
					break
				case 'when':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					Smruti.sendMessage(from, 'Question : *'+kapankah+'*\n\nAnswer : '+ koh, text, { quoted: mek })
					break
         			  case 'is':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					apakah = body.slice(1)
					...
					Smruti.sendMessage(from, 'Question : *'+apakah+'*\n\nAnswer : '+ kah, text, { quoted: mek })
					break
				case 'rate':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					rate = body.slice(1)
					...
					Smruti.sendMessage(from, 'Question : *'+rate+'*\n\nAnswer : '+ te+'%', text, { quoted: mek })
					break
                  ...
                  Smruti.sendMessage(from, anu, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nhow is it?`, quoted: mek})
                   break
                           case 'wolf':
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} Smruti*`)
                   F = body.slice(6)
                   reply(mess.wait)
...
if (!isGroup) return reply(`*Only group*`)
             ...
             let online = [...Object.keys(Smruti.chats.get(ido).presences), Smruti.user.jid]
             Smruti.sendMessage(from, '*Online list:*\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
      case "sider":
        ...
        let members = m.quoted.chat.endsWith("g.us")
          ? (await Smruti.groupMetadata(m.quoted.chat)).participants.length - 1
          : m.quoted.chat.endsWith("@broadcast")
          ? -1
          : 1;
        let { reads, deliveries } = await Smruti.messageInfo(
          m.quoted.chat,
          m.quoted.id
        );
        let txt = `
...
`.trim();
        m.reply(txt, null, {
          contextInfo: {
            mentionedJid: Smruti.parseMention(txt),
          },
        });
        break;
      case "q":
        if (!m.quoted) return reply("reply message!");
        let qse = Smruti.serializeM(await m.getQuotedObj());
        if (!qse.quoted)
      ...
      case "kick":
        ...
        if (mentioned.length > 1) {
          Smruti.groupRemove(from, mentioned);
          reply(mess.success);
        } else if (mentioned.length < 1) {
          anu = mek.message.extendedTextMessage.contextInfo.participant;
          Smruti.groupRemove(from, [anu]);
          reply(mess.success);
        } else {
          Smruti.groupRemove(from, mentioned);
          reply(mess.success);
...
case 'igstory': 
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    Smruti.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    Smruti.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
case 'linkwa':
      ...
      case "getbio":
        ...
        var p = await Smruti.getStatus(`${yy}`, MessageType.text);
        reply(p.status);
        if (p.status == 401) {
        ...
        case 'getdeskgc':
              if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
				if (!isGroup) return reply('this feature is only for groups')
					anu = from
			   metadete = await Smruti.groupMetadata(anu)
				Smruti.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
      // Get Name
      case "getname":
        var ambl = mek.message.extendedTextMessage.contextInfo.participant;
        const sname =
          Smruti.contacts[ambl] != undefined
            ? Smruti.contacts[ambl].sname || Smruti.contacts[ambl].notify
            : undefined;
        reply(sname);
        break;
      case "setdesc":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        Smruti.groupUpdateDescription(from, `${args.join(" ")}`);
        Smruti.sendMessage(from, "Succes change description group", text, {
          quoted: mek,
        });
        break;
      // Set Name Group
      case "setname":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        Smruti.groupUpdateSubject(from, `${args.join(" ")}`);
        Smruti.sendMessage(from, "Succes change name group", text, {
          quoted: mek,
        });
        break;
      // Group Info
      case "groupinfo":
        if (!isGroup) return;
        ppUrl = await Smruti.getProfilePicture(from); // leave empty to get your own
        buffergbl = await getBuffer(ppUrl);
        Smruti.sendMessage(from, buffergbl, image, {
          quoted: mek,
      ...
      case "demote":
        ...
        Smruti.groupDemoteAdmin(from, [mentionede]);
      ...
      case "promote":
        ...
        if (
          ...
          return reply("Reply members");
        mentionede = mek.message.extendedTextMessage.contextInfo.participant;
        Smruti.groupMakeAdmin(from, [mentionede]);
...
if (!Number(oi2)) return reply('Amount must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  Smruti.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Most!')
	  ...
	  Smruti.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await Smruti.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Amount must be a number!')
	if (Number(args[0]) >= 50) return reply('Most!')
	  for (let i = 0; i < args[0]; i++) {
	  Smruti.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await Smruti.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Amount must be a number!')
	if (Number(args[0]) >= 50) return reply('Most!')
	  for (let i = 0; i < args[0]; i++) {
	  Smruti.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
...
if (Number(oi2) >= 50) return reply('Most!')
	if (!Number(oi2)) return reply('Amount must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  Smruti.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
      case "revoke":
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
          ...
          media = await Smruti.downloadAndSaveMediaMessage(encmedia);
          await Smruti.sendMessage(
            from,
            `── 「 REMINDER 」 ──
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              Smruti.sendMessage(from, fs.readFileSync(media), sticker);
              _reminder.splice(
                reminder.getReminderPosition(sender, _reminder),
                1
              );
...
➸ Message: ${messRemind}
➸ Type: ${reminder.getReminderType(sender, _reminder)}`;
              Smruti.sendMessage(from, media, image, {
                contextInfo: { mentionedJid: [sender] },
                caption: teks,
              });
              _reminder.splice(
...
➸ Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              Smruti.sendMessage(from, fs.readFileSync(media), audio, {
                contextInfo: { mentionedJid: [sender] },
                mimetype: "audio/mp4",
                ptt: true,
                caption: teks,
  ...
  if (!mek.key.fromMe) return;
              ...
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   Smruti.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                Smruti.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                Smruti.groupUpdateDescription(from, `_${pushname} hacked this group_`)             
                await sleep(1000)
                Smruti.sendMessage(from, 'Succes Hacked', text, {quoted: fyt})
case 'mygithub':
  reply('https://github.com/DGSmruti')
					break;
   case 'ss':
      reply(mess.wait)
   ...
   sendButVideo(from, kodo, `Click Next To Continue`, get_result, [                      
          {
            ...
            type: 1,
                    ...
                    break    
case '+91':
                    get_result = await getBuffer(`https://Smruti.herokuapp.com/api/asupan?apikey=${valkey}`)
                    pll = `Intake`
...
filsize = args[0]
costick = await Smruti.prepareMessageFromContent(from,{
"stickerMessage": {
"url": m.quoted.url,
...
Smruti.relayWAMessage(costick)
break

case 'sizeimg':
if (args.length < 1) return reply('Enter the numbers')
filsizei = args[0]
costick3 = await Smruti.prepareMessageFromContent(from,{
"imageMessage": {
	"url": m.quoted.url,
	"mimetype": m.quoted.mimetype,
...
*Model* : ${i.phone.device_model}\n\n`;
	...
	case 'chat':
            ...
            var org = pc.split("|")[1];
            Smruti.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
            reply(`Sukses mengirim chat:\n${org},@${nomor}`)
            break
case 'viewonce':
res = await Smruti.prepareMessageFromContent(from,{
"viewOnceMessage": {
"message": {
"imageMessage": {
"mimetype": 'image/jpeg',
      ...
      case "instagram":
        ...
        hx.igdl(args[0]).then(async (result) => {
          for (let i of result.medias) {
            if (i.url.includes("mp4")) {
              let link = await getBuffer(i.url);
              Smruti.sendMessage(from, link, video, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            } else {
              let link = await getBuffer(i.url);
              Smruti.sendMessage(from, link, image, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            }
...
case 'setprefix':
    ...
    Follower: ${tod.result.user_followers}`;
        Smruti.sendMessage(from, buff, image, { quoted: mek, caption: tt });
        break;*/
    ...
    Follower: ${i.result.stats.followerCount}`;
        Smruti.sendMessage(from, buff, image, { quoted: mek, caption: ii });
        break;
   
  case 'writebook':            
            if (args.length == 0) return reply(`example: ${prefix + command} Smruti`)
            reply(mess.wait)
            teks = args.join(" ")
            ini_buf = await getBuffer(`https://Smruti.herokuapp.com/api/maker/nulis?apikey=${valkey}&text=${teks}`)  
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List🔖`},type:1}]
              imageMsg = (await Smruti.prepareMessageMedia(ini_buf, "imageMessage", { thumbnail: ini_buf, })).imageMessage
              buttonsMessage = {footerText:'WHATSAPP BOT', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Smruti.relayWAMessage(prep)
...
sendButMessage(from, tiyo, `WHATSAPP BOT`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             if (isMedia && !mek.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
             bc = await Smruti.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             Smruti.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 WHATSAPP BOT BROADCASTING 」*\n\n${body.slice(4)}`})
}
             reply('Broadcast success')
             } else {
             for (let _ of anu) {
Smruti.sendMessage(_.jid, 
...
buttons = [{buttonId: `yuri`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Smruti.prepareMessageMedia(kon, "imageMessage", { thumbnail: kon, })).imageMessage
              buttonsMessage = {footerText:'WHATSAPP BOT', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Smruti.relayWAMessage(prep)
...
buttons = [{buttonId: `anal`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Smruti.prepareMessageMedia(aku, "imageMessage", { thumbnail: aku, })).imageMessage
              buttonsMessage = {footerText:'WHATSAPP BOT', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Smruti.relayWAMessage(prep)
...
buttons = [{buttonId: `lesbian`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Smruti.prepareMessageMedia(kau, "imageMessage", { thumbnail: kau, })).imageMessage
              buttonsMessage = {footerText:'WHATSAPP BOT', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Smruti.relayWAMessage(prep)
...
buttons = [{buttonId: `eroneko`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Smruti.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'WHATSAPP BOT', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Smruti.relayWAMessage(prep)
...
buttons = [{buttonId: `bj`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Smruti.prepareMessageMedia(yoiz, "imageMessage", { thumbnail: yoiz, })).imageMessage
              buttonsMessage = {footerText:'WHATSAPP BOT', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Smruti.relayWAMessage(prep)
...
buttons = [{buttonId: `kitsune`,buttonText:{displayText: `➡️Next`},type:1}]
              ...
              buttonsMessage = {footerText:'WHATSAPP BOT', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
              Smruti.relayWAMessage(prep)
break
case 'pussy':
if (!isGroup) return reply('this feature is only for groups')
...
Smruti.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./Smruti.jpg')})
break
case  'blowjob':
if (!isGroup) return reply('this feature is only for groups')
if (!isNsfw) return reply(`Nsfw feature is not yet active in this group\nType: ${prefix}nsfw 1 \To activate`)
...
Smruti.sendMessage(from, anu, text, {quoted: mek})
break
case 'xvideo':
case 'xv': 
...
Smruti.sendMessage(from, vid, video, {quoted: mek})
break
case 'writelist':
  reply(`Example ${prefix}write tes
  🌿 ${prefix}write Smruti
  🌿 ${prefix}folioright Smruti
  🌿 ${prefix}folioleft Smruti
  🌿 ${prefix}writedown Smruti
  🌿 ${prefix}writeleft Smruti`)
  break
case 'folioleft':
if (!c) return reply('Where is the text bro??')
reply(mess.wait)
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply('only admin can use this feature')
					...
					if (Number(args[0]) === 1) {
						...
						Smruti.sendMessage(from, `Free to Find Hentai 🗿`, text)
			...
			case 'maker3d': 
					...
					Smruti.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./Smruti.jpg')})
					break
			case 'maker3d2': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} Smruti`)
					makell = args.join(" ")
...
buttons = [{buttonId: `quote`,buttonText:{displayText: `Thx Dah Pake`},type:1}]
              imageMsg = (await Smruti.prepareMessageMedia(buffer1, "imageMessage", { thumbnail: buffer1, })).imageMessage
              buttonsMessage = {footerText:'WHATSAPP BOT', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Smruti.relayWAMessage(prep)
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await Smruti.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						Smruti.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:fvn})
				...
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						Smruti.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: fvid})
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ger = isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          owgi = await Smruti.downloadAndSaveMediaMessage(ger);
          Smruti.sendMessage(sender, fs.readFileSync(owgi), "imageMessage", {
            caption: q,
          });
          reply("Success");
          fs.unlinkSync(owgi);
          ...
          ger = isQuotedVideo
            ...
            : mek;
          owgi = await Smruti.downloadAndSaveMediaMessage(ger);
          Smruti.sendMessage(sender, fs.readFileSync(owgi), "videoMessage", {
            caption: q,
          });
          reply("Success");
          fs.unlinkSync(owgi);
      ...
      case "caripesan":
        ...
        let v = await Smruti.searchMessages(q, from, 10, 1);
        let s = v.messages;
        ...
        try {
          ...
          for (let i = 0; i < el.length; i++) {
            await Smruti.sendMessage(from, "Here is the message", text, {
              quoted: el[i],
            });
          }
         } catch (e) {
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`;
        ...
        Smruti.sendMessage(from, ram, image, { quoted: mek, caption: rem });
        break;
      case "comic":
...
*Final Chapter* : ${komik.chapter_akhir}`;
      ...
      case "chara":
        ...
        await Smruti.sendMessage(from, li, image, { quoted: mek });
        break;
      case 'pinterest':
...
buttons = [{buttonId: `pinterest`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await Smruti.prepareMessageMedia(di, "imageMessage", { thumbnail: di, })).imageMessage
              buttonsMessage = {footerText:'WHATSAPP BOT', imageMessage: imageMsg,
              contentText:`ɴᴏᴛᴇ: ʜᴏʀɴʏ = ʙᴏɴᴋs`,buttons,headerType:4}
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              Smruti.relayWAMessage(prep)
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak});
        ...
        if (
          ...
          encmedia = isQuotedSticker
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          file = await Smruti.downloadAndSaveMediaMessage(
            encmedia,
            (filename = getRandom())
          );
          value = args.join(" ");
          var group = await Smruti.groupMetadata(from);
          ...
          Smruti.sendMessage(from, ini_buffer, sticker, options);
          fs.unlinkSync(file);
        } else {
      ...
      case "totag":
        if (
          ...
          ini_buffer = fs.readFileSync(file);
          Smruti.sendMessage(from, ini_buffer, image, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          Smruti.sendMessage(from, ini_buffer, audio, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          Smruti.sendMessage(from, ini_buffer, video, options);
          fs.unlinkSync(file);
        } else {
          reply(
      ...
      case "slander":
        ...
        var bot = gh.split("|")[2];
        Smruti.sendMessage(from, `${bot}`, text, {
          quoted: {
            key: {
              fromMe: false,
      ...
      case "slanderpc":
        ...
        const responye = await Smruti.sendMessage(
          jids,
          `${split[1]}`,
          MessageType.text,
          options
        );
        await Smruti.deleteMessage(jids, {
      ...
      case "tomp3":
        ...
        exec(`ffmpeg -i ${media} ${ran}`, (err) => {
          ...
          Smruti.sendMessage(from, buffer453, audio, {
            mimetype: "audio/mp4",
            quoted: mek,
          });
          fs.unlinkSync(ran);
      ...
      case "fast":
        ...
        exec(
          ...
          (err) => {
            ...
            if (err) return fakegroup(`Err: ${err}`);
            buffer453 = fs.readFileSync(ran);
            Smruti.sendMessage(from, buffer453, video, {
              mimetype: "video/mp4",
              quoted: mek,
            });
            fs.unlinkSync(ran);
  ...
  case 'buttons5':
          ...
          case 'dare':
              ...
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{contextInfo: { mentionedJid: [sender]},quoted:ftex})
              Smruti.relayWAMessage(prep)
              break
      case "anime":
        reply(mess.wait);
...
case 'waifu':
          ...
          case 'wallml':
              ...
              imageMsg = ( await Smruti.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              ...
              prep = await Smruti.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              Smruti.relayWAMessage(prep)
...
Link : ${get_resultP.url_audio}
            `
            Smruti.sendMessage(from, textP, text,{contextInfo:{
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
            ...
            Smruti.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_resultP.title}.mp3`, quoted: mek})
            break
case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} Smruti`)
              buffer = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURI(q)}`)
              Smruti.sendMessage(from, buffer, sticker, { quoted: freply })
              break
					case 'githubstalk':
					...
					Smruti.sendMessage(from, buff, image, {quoted: ftok, caption: gstalk})
					break 
					case 'infoearthquake':
					...
					Smruti.sendMessage(from, buff, image, {quoted: ftok, caption: gempa})
					break 
      case "contact":
        pe = args.join(" ");
        entah = pe.split("|")[0];
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        require("./lib/fetcher.js").modStick(media, Smruti, mek, from);
        break;
      case "stikerwm":
      case "stickerwm":
      case "swm":
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ...
          ffmpeg(media)
            .on("error", (e) => {
              console.log(e);
              Smruti.sendMessage(from, "Terjadi kesalahan", "conversation", {
                quoted: mek,
              });
              fs.unlinkSync(media);
            })
          ...
          const encmedia = isQuotedVideo
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          const media = await Smruti.downloadAndSaveMediaMessage(encmedia);
          pe = args.join("");
          var a = pe.split("|")[0];
          var b = pe.split("|")[1];
          await createExif(a, b);
          ...
          ffmpeg(media)
            .on("error", (e) => {
              console.log(e);
              Smruti.sendMessage(from, "There is an error", "conversation", {
                quoted: mek,
              });
              fs.unlinkSync(media);
            })
      ...
      case "fdeface":
        ...
        const dipes =
          isQuotedSticker || isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
        const tipes = await Smruti.downloadAndSaveMediaMessage(dipes);
        ...
        var mat = await Smruti.generateLinkPreview(url);
        mat.title = title;
        mat.description = desc;
        mat.jpegThumbnail = bufer;
        mat.canonicalUrl = buu;
        Smruti.sendMessage(from, mat, MessageType.extendedText, anu);
        break;
      case "public":
        if (!mek.key.fromMe) return reply("This feature is only for the owner 🗿");
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak});
        ...
        Smruti.sendMessage(from, optionshidetag, text);
        break;
...
*[Wait] Wait a moment..*`
      ...
      case "s":
        if (
          ...
          await ffmpeg(`./${media}`)
            ...
            .on("end", function () {
              console.log("Finish");
              Smruti.sendMessage(from, fs.readFileSync(ran), sticker, {
                quoted: mek,
              });
              fs.unlinkSync(media);
              fs.unlinkSync(ran);
      ...
      case "ytsearch":
        ...
        try {
          ...
          return await Smruti.sendMessage(
            from,
            "Error!",
            MessageType.text,
            dload
      ...
      case "join":
        try {
          ...
          var response = await Smruti.acceptInvite(codeInvite);
          grupinv("SUCCESS");
        } catch {
          ...
          boij =
            isQuotedImage || isQuotedVideo
              ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                  .extendedTextMessage.contextInfo
              : mek;
          owgi = await Smruti.downloadMediaMessage(boij);
...
${
  descOwner
    ...
    : "*Desc modified by* : -"
          ...
          Smruti.sendMessage(from, par, text, {
            quoted: mek,
...
exec(q, (err, stdout) => {
if (err) return Smruti.sendMessage(from, ` ${err}`, text, { quoted: mek })
if (stdout) {
Smruti.sendMessage(from, stdout, text, term)
}
})
}
